...
CMD ["/bin/sh", "-c", "PUID=${PUID:-911} PGID=${PGID:-911} && [[ -z ${LSIO_READ_ONLY_FS} ]] && [[ -z ${LSIO_NON_ROOT_USER} ]] && { groupmod -o -g \"$PGID\" abc; usermod -o -u \"$PUID\" abc; }; exec su abc -c './discord-bot'"]